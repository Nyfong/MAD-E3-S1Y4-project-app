...
void main() async {
  ...
  const String baseUrl = 'https://api-mad.procare.sbs/api/v1';
  const String endpoint = '/recipes/bulk';
  ...
  final recipes = [
    // Asean category
    {
      ...
      "instructions": [
        "Step 1: Heat oil in a pan and saute garlic until fragrant.",
  ...
  // Create recipes
  print('Creating ${recipes.length} recipes...\n');
  int successCount = 0;
  int failCount = 0;
  ...
  for (int i = 0; i < recipes.length; i++) {
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        print('✗ Failed to create ${recipe['title']}: Status ${response.statusCode} - $responseBody');
        failCount++;
      }
    } catch (e) {
